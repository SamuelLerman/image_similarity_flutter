...
def convnet_model_():
    ...
    x = Dense(4096, activation='relu')(x)
    x = Dropout(0.6)(x)
    x = Lambda(lambda  x: normalize(x))(x)
...
def deep_rank_model():
    ...
    first_max = Lambda(lambda x: normalize(x))(first_max)

    second_input = Input(shape=(224,224,3))
    second_conv = Conv2D(96, kernel_size=(8, 8),strides=(32,32), padding='same')(second_input)
    second_max = MaxPool2D(pool_size=(7,7),strides = (2,2),padding='same')(second_conv)
    second_max = Flatten()(second_max)
    second_max = Lambda(lambda x: normalize(x))(second_max)

    merge_one = concatenate([first_max, second_max])

    merge_two = concatenate([merge_one, convnet_model.output])
    emb = Dense(4096)(merge_two)
    l2_norm_final = Lambda(lambda x: normalize(x))(emb)
...
def are_images_similar(image_path, ref_images_directory_path, model, threshold):
    for ref_path in ref_images_directory_path:
      x = compare_images(ref_path, image_path, model)
      if x < threshold:
          return True
    return False
